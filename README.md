...
- 👥 **客户端管理** - 添加、编辑、删除、启用/禁用客户端
- 🟢 **实时在线状态** - 客户端在线状态实时监测，绿色呼吸灯表示在线
- 🌐 **自定义 IP** - 支持手动指定客户端 IP 地址或自动分配
- 📄 **配置下载** - 一键生成客户端配置文件
- 📱 **二维码连接** - 扫码即可连接 VPN
- 📥 **配置导入** - 支持从系统现有配置文件导入
- ⚡ **热更新** - 添加/删除/禁用客户端无需重启 WireGuard 服务
- 🌙 **主题切换** - 支持暗黑/亮色主题
- 🔐 **安全认证** - JWT Token 登录认证
...
4. 可选：指定 IP 地址（如：10.0.8.100/32），留空则自动分配
5. 系统自动生成密钥对和 PresharedKey
...
| 操作 | 说明 |
|------|------|
| 🟢/🔴 状态指示 | 绿色呼吸灯表示在线，红色表示离线 |
| ✏️ Edit | 编辑客户端名称 |
| 🔄 Enable/Disable | 启用或禁用客户端（实时生效） |
| ⬇️ Download | 下载客户端配置文件 |
| 📱 QR | 显示连接二维码 |
| 🗑️ Delete | 删除客户端（实时生效） |
